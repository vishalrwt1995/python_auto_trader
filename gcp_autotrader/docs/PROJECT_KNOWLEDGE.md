...
### ★ NEW 2026-08-24 — extract the 4 identical `_resolve_instrument_keys` copies into one helper (mechanical, low value, non-zero risk)
`pead`/`delivery`/`insider`/`pledge` each carry a byte-identical copy of the fresh+deep key-resolution union (differing only in the log prefix — asserted programmatically in ㉚). Four copies of one function is the same duplication shape that let the EOD-exemption bug ship **twice** (CORE 06-22, delivery 07-16), so this is worth closing.

Deliberately deferred when the union shipped: extracting it touches four **live entry paths** at once, including `delivery` — the only channel with realized profit. The risk of a refactor breaking four channels simultaneously was judged larger than the risk of the duplication, given no behavioural question remains open.

**Now purely mechanical:** move one function to a shared module, parameterise the log prefix, update four call sites, assert 1105 tests unchanged. No design decision left. Do it as its own PR with nothing else in it, so a regression is unambiguously attributable.
