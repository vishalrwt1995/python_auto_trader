...
Candidate fixes (undiscussed — pick before building):
1. **Add a Monday-morning ingest** before the 09:12 scan (e.g. `08:55`), or shift the scan later than the ingest.
2. **Enable weekend ingests** (`* * 6,0`) so Sat/Sun filings land on their own date.
3. **Accept it** and quantify the cost first — needs a count of weekend-dated qualifying revokes/clusters over
   the backtest window. If it is ~0-1/yr this is not worth the deploy risk.

Option 3 first: **the cost is currently unmeasured.** Do not ship 1 or 2 before counting how many real signals
this has actually cost, since both touch live scheduling for the two newest channels. Note also that the
backtest never modelled ingest timing at all — it iterates every date — so any fix here *improves* prod-vs-
backtest parity rather than threatening it.
