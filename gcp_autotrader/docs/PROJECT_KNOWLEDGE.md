...
- **stop anchored to the REFERENCE close, not the fill** — solving `sl=e−d` / `target=e+20d` gives `d=401,
  ...
  insider. ✅
- ⚠️ **Still unexercised: the EOD exemption.** It must survive the 15:20 reconcile un-squared; pledge is in
  both lists per PR #69 but that has never run with a live pledge position. **Check this.**
