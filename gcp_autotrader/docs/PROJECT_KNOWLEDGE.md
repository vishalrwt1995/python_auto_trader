...
### 0. CORE channel ~18% cash drag at ₹3L (new 2026-06-21, decide next session)
The first CORE rebalance deployed only ₹246k of ₹3L (**₹54k / 18% idle**) because integer-share equal-weight sizing at a ₹10k/name slice can't (a) buy 1 share of names >₹10k (MARUTI/POLYCAB/BAJAJ-AUTO skipped) nor (b) avoid large rounding residuals on names priced ₹4,500-8,500 (1 share each). Backtest assumed fractional equal-weight → this is a live-only granularity gap pulling ~9-10% → ~7.5-8%. **Proposed fix: residual-cash sweep** — after the equal-weight pass, buy additional shares of the lowest-priced held names until cash is ~exhausted (mildly overweights cheap names, deploys ~98%, stays close to equal-weight). ~10-line change in `core_trading_service.plan_core_rebalance` + a unit test + fidelity re-check. Alternatively reduce TOPN (bigger slices) — but deviates from the validated top-30. Channel works today; this is an efficiency fix, not a correctness bug. **NOT yet shipped — awaiting user go.**
...
### 2026-06-21 (latest) — CORE channel SHIPPED + ENABLED (PR #34, revs autotrader-00270-llh + ws-monitor-00044-rmw, PAPER, ₹3L NEW)
...
- **CAVEAT (honest) — ~18% cash drag at ₹3L:** integer-share sizing deployed only ₹246k of ₹3L → **₹54k idle**. 3 names >₹10k/slice skipped entirely (MARUTI ₹13,395 / POLYCAB ₹10,083 / BAJAJ-AUTO ₹10,066, all logged `core_skip_qty_zero`); + big rounding residual on the ~6 names priced ₹4,500-8,500 that get only 1 share (DIVISLAB ₹6,750 → 1 sh, ₹3,250 idle). Backtest assumed fractional equal-weight (full deployment), so this is a live-only granularity gap that pulls ~9-10% → ~7.5-8%. **Fix = residual-cash sweep (Open Items §7) — not yet shipped; channel works, drag is logged + understood.**
