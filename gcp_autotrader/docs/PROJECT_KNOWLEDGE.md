...
### I. EVENT-channel corp-action (bonus/split) sub-strategy — LOCKED spec, build pending
Validated 2026-06-20 (see §8 + `docs/EVENT_CHANNEL_CORP_ACTION_PLAN.md`). Bonus/split pre-meeting run-up, **+2.48% net/event** liquid (robust IS+OOS), ~+2.4–3.4%/yr added to the EVENT channel **uncorrelated with PEAD**. Build = 2nd sub-strategy inside the EVENT/PEAD channel (own intimation-entry + hard meeting-exit, shared ₹2L/5-slot pool, **2-slot cap**, PAPER). Sequence after PEAD's first live trades, or build fully isolated. **Awaiting user go** on build-vs-pivot — proposed alternative next thread is the **intraday 5m deep-edge hunt** (likely the bigger untapped lever).
...
### 2026-06-20 (latest) — Corporate-action edge sweep + bonus/split LOCKED (RESEARCH, no prod change)
