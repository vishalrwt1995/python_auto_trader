...
- **Per-channel capital roster (verified 2026-07-14):** `CAPITAL_SWING=500000` + `CAPITAL_INTRADAY=100000` (HALTED) + `CAPITAL_PEAD=200000` + **`CAPITAL_CORE=300000`** + **`CAPITAL_MOMENTUM=200000`** = **₹13L across active channels** (shared fallback `CAPITAL=600000` unchanged). **`GAPFADE_MAX_POSITIONS=0` + `CAPITAL_GAPFADE=0` — gapfade KILLED 2026-07-14** (no OOS edge; ₹1L freed, left unallocated). `CORE_ENABLED=true` + `MOMENTUM_ENABLED=true`. PAPER throughout.
...
### 2026-07-14 (latest) — ⑱ GAPFADE KILLED + FULL CHANNEL DEEP-GRIND REVIEW (env rev `autotrader-00291-h2p` + dashboard PR #62 rev `00072-5fc`, PAPER)
