...
**Open (needs the user):** ⚠️ **Upstox WS token expired** → `autotrader-ws-monitor` is crash-looping on `ConnectionClosedError`. Contained (hold channels exit via the daily reconcile jobs, all 200), but it blocks live intraday marks and live tick exits — **needs a re-auth**. ⚠️ **`candles_1d` rot**: its `score-cache-update-close` writer stopped populating the universe ~April; nothing live depends on it now (channels fetch bars via Upstox directly) but it is real. ℹ️ Also noted: `tests/test_market_brain_v2.py` fails collection on a removed `GrowwSettings` import (pre-existing; the documented test command already ignores it).
