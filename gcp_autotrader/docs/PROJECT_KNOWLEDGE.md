...
### K. Swing 9.7% config — PAPER forward-validation (ACTIVE 2026-07-03; unblocked 2026-07-08)

**2026-07-08:** the forward-validation was silently blocked — swing traded 0 for 8 days, root-caused to a brain RECOVERY-lock (NOT the config; see §8 ⑪), now fixed + deployed (rev `00282-p4b`) + regime unlocked (`state.regime` RECOVERY→RANGE). Validation can finally proceed at the next open.
...
3. Liquidity cap binding on thin names (or `swing_liq_cap_no_turnover` warnings → plumbing gap to fix).
...
**Residual / follow-ups (optional, not blocking):** (1) the EOD/weekend dq window is not gated (the fix covers the full trading window; unreachable today since the brain doesn't persist post-close) — clean hardening is `phase != "LIVE"`. (2) A stuck-regime / no-trades monitor is recommended — the freeze went unnoticed for 8 days; an alert would catch any recurrence in a day.
