...
2. **Regime-first fetch optimization** — `run_momentum_rebalance_once` fetches all ~1,096 daily histories *before* the regime gate, wasting ~1,096 Upstox calls + ~2 min whenever the regime is cash. NOT a correctness bug (0 orders is correct). Fix: check `regime_ok` first; if False + no holdings to sell, skip `fetch_universe_history` (basket = empty). Single-service deploy (`autotrader` only). Memory `project_momentum_channel`.
...
### 2026-07-10 (latest) — ⑰ MOMENTUM × LOW-VOL CHANNEL SHIPPED (PR #59, revs `autotrader-00289-ftq` / `ws-monitor-00045-j2s`, PAPER, ENABLED)
