...
* `autotrader-insider-ingest-1930` cron **WAS** **`30 19 * * 1-5`** — Mon-Fri only, no Sat/Sun run. *(This bullet is the original diagnosis; the cron was changed to `30 19 * * *` on 2026-08-25 — see the APPLIED + VERIFIED status further down. Left in place because the reasoning below only makes sense against the broken state.)*
...
### ★ OPEN 2026-08-25 — re-attribute this repo's git history to `vishalrwt1995`? (decision, not a task)
Going forward is fixed (§8 ㉜). The whole existing history is still attributed to `vishal01012`,
because `vishal@eloelo.in` is linked to that account on GitHub.

Two ways to change that, and the **second is better**:
1. **Rewrite history + force-push `main`.** Breaks every worktree and clone, rewrites shipped
   commit SHAs that this doc and the §8 entries reference by hash. **Not recommended** — pure
   attribution cosmetics at real cost.
2. **Add `vishal@eloelo.in` as a verified email on the `vishalrwt1995` GitHub account.** GitHub
   then re-attributes the entire history server-side, zero rewriting, no SHA churn.

Option 2's catch: it moves **all** eloelo-authored commits everywhere on GitHub to
`vishalrwt1995`, including the voxy/StoryTV work — which may not be wanted. Needs a deliberate
call while logged into GitHub; not something to do on the way past.

**Or do nothing.** Attribution has no effect on the trading system. Filed only so it is a choice
...
fix) + `db82c01` (exit-position fix). ws `00048-b9g` · dashboard `00084-pgl` unchanged. PAPER.
Single-service both times (Rule 8 N/A). Both verified from `status.traffic[0].revisionName`, not
exit code, and both checked for startup ERRORs before being called done.

**Bug 1 — MON100 found live, and Friday's fix (㉟) had one gap left.** A routine "check today's
run e2e" cross-referenced all **62** open positions' real `instrument_key` against
`etf_filter.is_non_equity()` directly — not the symbol names, the ISIN — and found exactly one
violation: `MON100` (`NSE_EQ|INF247L01AP3`), open in the momentum channel since **2026-08-01**,
before any ETF fix existed. `git log -S"is_etf(str(sym))"` on `momentum_signal_service.py` showed a
**name-layer** block was added **2026-08-04** (`bc4ede4a`, comment names MON100 explicitly) — so
the open position was already a dead-by-name leftover, not a live-recurring risk. But reading
Friday's fix (73a14c0/545dafc) against momentum's source showed the **ISIN-layer** fail-safe
(`split_fund_keys`, the layer that catches a fund whose *ticker* matches no curated name or
pattern — the exact class of bug that nearly let `INF740KA1SW3` through delivery) was applied to
**5** channels and never to momentum. Fixed with the identical call, same log-line convention, at
`momentum_trading_service.py:108` — right after `mss.fetch_universe(state)`, before any history
fetch. New test drives `run_momentum_rebalance_once` through a mixed equity+fund keymap and
asserts the fund never reaches `fetch_universe_history` (the existing
`test_build_target_basket_excludes_etf_even_when_top_ranked` already covered the name layer; this
was the missing ISIN layer). **All 6 channels now have both layers.**

**Bug 2 — trying to exit MON100 exposed the admin endpoint had probably never worked.** Clicking
"Exit Position" in the dashboard did nothing visible. Checked prod logs before touching code: two
live `500`s at 08:42 IST, `admin/exit-position failed: 'function' object has no attribute
'place_exit_order'`. `container.py:142` defines `order_service` as a lazy-init **factory method**
— every one of its other 14+ call sites correctly calls `self.order_service()`.
`post_admin_exit_position` (`dashboard_api.py`) was the one place calling it as a bare attribute,
so `c.order_service` returned the method object itself and `.place_exit_order` failed. **Zero test
coverage on this endpoint, and prod logs show no prior successful call ever** — this was very
likely the first time a human had ever tried a manual exit through the dashboard. Fixed: added the
missing `()`. New regression test (`test_dashboard_admin.py`) drives the real endpoint function
against a fake container shaped like the real one (`order_service` as a callable) and asserts
`place_exit_order` is actually invoked; separately confirmed by hand that the pre-fix call pattern
reproduces the *exact* prod `AttributeError` message before trusting the test had teeth.

**Also found, NOT fixed (separate deploy target, flagged not actioned):** the dashboard frontend's
`handleExitPosition` (`dashboard/src/app/positions/page.tsx:59-60`) swallows every error from the
exit call in an empty `catch {}` — no toast, no console log, nothing. That's *why* Bug 2 looked
like "nothing happened" instead of surfacing as a visible failure. Left open — Next.js dashboard is
a separate deploy pipeline this session never touched, and it wasn't blocking (the backend fix
alone makes the action work); worth fixing next time dashboard work is in scope.

**Verified end-to-end, not just deployed.** MON100 exited via the now-fixed dashboard action:
Firestore confirms `status=CLOSED`, `exit_reason=MANUAL_EXIT`, `exit_price=331.54`,
`pnl=+₹833.60`, `exit_ts=2026-08-31T15:41:22+05:30`. 61 open positions remain.

1134 → 1136 tests (2 new, one per bug). Suite green both times, zero regressions. Also hit the
recurring stale-`gh`-identity 403 (`vishal01012` active instead of `vishalrwt1995`) on **three**
separate pushes this session — `gh auth switch --user vishalrwt1995` fixed it each time, but it
had silently reverted to `vishal01012` between the first and second push, most likely another
concurrent session on this shared machine touching `gh`'s global active-account state. Not
investigated further; switching immediately before each push worked every time.

### ㉟ 2026-08-28 · ★ TWO LIVE STOCK-ONLY VIOLATIONS — three drifted ETF filters + a fund ISIN that reached the fetch
Deployed **`autotrader-00320-nxm`** · commits `73a14c0` (fix) + `545dafc` (consolidation).
ws `00048-b9g` · dashboard `00084-pgl` unchanged. PAPER. Single-service (Rule 8 N/A). Startup
verified from `status.traffic[0].revisionName`, not the exit code.

**Found from ONE error log** during a routine daily check:
`delivery_fetch_symbol_failed key=NSE_EQ|INF740KA1SW3 err=HTTP 400 UDAPI100011`.

**★ DEFECT 1 — three DRIFTED copies of `is_etf`, and the 08-07 fix never reached two channels.**
`delivery_signals` carried **29** curated names; `insider_signals` and `pledge_signals` carried
**13** (identical to each other). insider/pledge were therefore missing **20 names, none of which
match any name pattern**: `GOLDSHARE, LIQUIDCASE, LIQUIDADD, MAFANG, N100, QGOLDHALF, KOTAKGOLD,
KOTAKNIFTY, AXISGOLD, AXISNIFTY, TATAGOLD, GROWWGOLD, HDFCLIQUID, LIQUID, MASPTOP50, MOM30,
QNIFTY, UTINIFTETF, SETFNIFBK, SETFNIFTY`. **`LIQUIDCASE` is one of the exact ETFs ㉔ records as
having reached the SIGNAL stage on 08-07** — that fix landed in `delivery_signals` and
`universe_service` and was never propagated. **Both channels would have traded these today.**
(delivery lacked 4 that the others listed — all `*BEES`, already caught by the pattern — so
delivery loses nothing.)

**★ DEFECT 2 — name matching cannot be complete.** `NSE_EQ|INF740KA1SW3` passed BOTH name checks
in delivery (the pre-fetch filter at `delivery_trading_service.py:218` AND the gate at
`delivery_signals:87`, which call the same function) and reached `_fetch_symbol_daily`. **It was
stopped only because Upstox returned HTTP 400 — luck, not design.** Had the call succeeded it
would have proceeded to gates applying the same failed test. The ISIN was available the whole
time: **NSE company equity is `INE…`; mutual-fund units (an ETF is one) are `INF…`**.

**Fix.** New PURE `domain/etf_filter` as the single home for the STOCK-ONLY rule:
* `ETF_CURATED` = the **union** of all three lists (33). My hand transcription initially missed
  `ICICIB22/MOM100/MOM50` — caught by verifying against the real lists programmatically instead of
  trusting the typing.
* `is_etf_symbol` (name layer) · `is_fund_instrument_key` (ISIN layer) · `is_non_equity` (both).
* `split_fund_keys(key_map) -> (equity, fund_syms)` — PURE, applied in **all 5 channels** right
  after `resolve_instrument_keys`: the first moment an ISIN is known and **before any network
  call**. Logs `<channel>_stock_only_dropped_funds`.
* The three modules keep their public `is_etf` as a thin delegation, so every caller works
  unchanged (`delivery_signals:87`, insider `aggregate_legs`/`passes_insider_gates`, pledge gates,
  `momentum_signal_service`, `universe_service`).
* Deliberately **NOT** placed inside the shared resolver: that module's equivalence was proven
  byte-for-byte in PR #73, and mixing stock-only POLICY into a resolver would undo that guarantee.

**BLAST RADIUS — verified, not assumed.** `momentum_signal_service` and `universe_service` both
import from `delivery_signals`, so they gain only the 4 `*BEES` names already pattern-caught ⇒
**SWING, MOMENTUM, DELIVERY and CORE are behaviourally UNCHANGED.** Only insider and pledge change,
and only in the restrictive direction.

**SAFETY PROPERTIES, each checked:**
1. **ONE-DIRECTIONAL** — no symbol any old copy excluded is now allowed (verified across the full
   33-name union). Over-exclusion is tolerable for a stock-only mandate; letting a fund through is
   not.
2. **No HELD position affected** — all 11 open names still classify as equity (`HEG, THANGAMAYL,
   TRUALT, SILVERTUC, APOLLOHOSP, PARADEEP, ANURAS, AYE, CYIENT, HONAUT, MEDANTA`).
3. **FAIL-SAFE direction** — a missing/unparseable key **KEEPS** the symbol, so a resolver outage
   cannot silently empty a book. Mutation-tested: flipping this is caught.
4. **No mutation of `_ETF_CURATED`** anywhere, so aliasing it to a frozenset is safe.
5. All 5 trading services import; `autotrader.main` imports.

**15 tests, MUTATION-TESTED — and the first pass caught a VACUOUS test of mine.** "break the
`endswith("BEES")` pattern" **ESCAPED**, because every `BEES` name we happen to curate is also
pattern-matched, so the pattern was never actually exercised — a *newly-listed* `…BEES` ETF would
depend on it alone. Rewrote against non-curated names (`FOOBEES`); all 6 mutants then caught.
Suite **1119 → 1134**.

**A self-correction worth recording.** `73a14c0` added the same 7-line ISIN block to five
services — the exact duplication pattern this session spent its time removing (4× resolver, 3×
`is_etf`). Caught while reviewing my own commit before deploy; `545dafc` collapsed it into one
pure function. That also turned an untestable inline block into 5 real tests, including the exact
live `key_map` case and the fail-safe direction.

**⚠️ RESIDUAL RISK — two things synthetic tests cannot prove:**
1. The gate has **never run against a real prod `key_map`**. First proof is the 08-28→29 scans.
2. If a legitimate equity ever carried an `INF` ISIN it would now be dropped. Believed impossible
   (`INE` = company equity) but **not exhaustively verified against the NSE instrument master**.
   ⇒ **Watch `<channel>_stock_only_dropped_funds`: expect absent, or only genuine funds. A real
   stock appearing there is the REVERT signal** — code-only, no env, so reverting is redeploying
   the prior revision. Also expect `delivery_fetch_symbol_failed` to STOP recurring, since the
   fund is now dropped before the fetch.

### ㉞ 2026-08-27 · PEAD's silent reaction-date loss made MEASURABLE (deliberately not fixed yet)
Deployed **`autotrader-00319-jms`** · commit `40fd9ad`. ws `00048-b9g` · dashboard `00084-pgl`
unchanged. PAPER. Single-service (Rule 8 N/A). Clean startup verified from
`status.traffic[0].revisionName`, not the exit code.

**The defect (long known, filed in §7, still unfixed).** PEAD reads ONE reaction session per scan
(`reaction_target = nifty_daily[-1][0]`) and keeps only events reacting on exactly that day. No
catch-up, no idempotency ledger, and — unlike pledge/insider after PR #70 — **no log at all** when
a session is dropped. Confirmed by grep: `reaction_dates` usage is pledge **8** · insider **8** ·
pead **0**. So an event whose reaction session passes unprocessed is lost **silently**. The 5-day
`fetch_result_events` lookback does NOT provide catch-up — it only tolerates holiday gaps.

**★ WHY THE PORT WAS NOT SHIPPED.** I offered to port the PR #70 machinery and then stopped, for
two reasons worth preserving:
1. **It would recover almost nothing.** Entering a session that has already passed breaks backtest
   parity, so the port converts a SILENT loss into a LOGGED one — exactly what the weekend-ingest
   fix (㉛/§7) turned out to do. Its real value is observability, not recovered trades.
2. **Nobody knows whether pead loses anything at all.** Building catch-up machinery for an
   unmeasured loss is the same mistake the weekend-cost measurement was created to avoid.

**So: measure first.** One PURE classifier + two log lines:
```
pead_reaction_split target= on_target= past= future= nobars= events=
pead_reaction_past  n= unrecovered= unrecovered_syms=
```
**`unrecovered` is the honest number.** A symbol can hold two filings in the 5-day window — an
older one whose session has gone plus a newer one reacting today — and the selector trades it on
the newer filing, so that is NOT a loss. Only past events whose symbol was never traded count.
That inflation was caught during testing, before any number was collected.

**Verified by construction, not assertion.** The classifier mirrors the selector's loop including a
quirk a naive copy gets wrong: `seen` is populated only on a **MATCH**, so a symbol with several
filings is retried until one matches. A synthetic case covering on-target / past / future /
beyond-bars / nobars **plus that retry** proves `on_target` == the selector's output exactly and
that the buckets cover every symbol. 62 insertions / **0 deletions**. 1119 tests pass, 5 skipped —
unchanged. `autotrader.main` imports clean.

**DECISION RULE for the next session that reads this:**
* `unrecovered` consistently **0** ⇒ the single-date reader never cost anything. Downgrade the §7
  item to cosmetic and **do not build the port.**
* `unrecovered` recurring **> 0** ⇒ real silent loss, now quantified. The port becomes justified
  with a number attached, and `domain/reaction_dates.py` already exists, is pure, is tested, and
  has run live in pledge/insider for two weeks.

**Context — this is NOT why pead has no trades.** The market gate is: `nifty_dd −0.081` vs gate
`−0.05`, which zeroes candidates upstream of everything. Measured the same day: the gate is open
**56.7% of days** across 2015-2026 (2025 46%, 2026-to-May 40%) — so it is NOT structurally
restrictive like swing's b200>70 (36.4%, and 0.0% in 2026). But closed spells are heavily skewed:
**median 3 days, yet the tail runs 4-9.4 months** (198d 2015-16 · 153d 2020 · 138d 2024-25 · 115d
2018-19), and the current spell is **~5.6 months and counting**. The roll-off escape is dead — the
2nd-highest close in the window would still leave dd at −0.1004 — so **only a ~3.4% Nifty rally
opens it.** Correction to an earlier claim of mine: "it will activate on its own" quoted the
unconditional 57% and ignored the conditional-on-closed duration; the direction was right, the
implied imminence was not.

### ㉝ 2026-08-26 · SWING SCORER DECOMPOSED (P10) — the ordering role is REAL, the threshold roles are not, and the candidate DIED on the bar (P11)
No deploy. Read-only grind, local cache only (no BigQuery, zero GCP cost). Prod untouched —
`domain/`, `services/`, `settings.py` byte-unchanged; the only edits are additive knobs in the
grind harness. Live stays `autotrader-00318-ws6`.

**Why this ran.** ㉗ concluded swing's edge is *exit asymmetry, not selection*, on the evidence that
every recorded scorer is uncorrelated with outcome (`adj_score r=-0.006`, `raw_score r=-0.033`,
`wl_score r=+0.037`) and the LOWEST adj_score bucket outperforms the highest. ~49 arms had swept
every CONFIGURATION axis but nobody had asked whether the selection layer contributes anything at
all. New scripts `swing_p10_no_scorer.py`, `swing_p11_prefilter_plateau.py`.

**The scorer has THREE roles; they had to be separated or the result would be confounded:**
role 1 = component pre-filter (`comp < _emit_floor`) · role 2 = adj_score admission gate
(`adj_score < EMIT_FLOOR`) · role 3 = slot allocation (`sorted(cands, -wl_score)`).

**★ P10 RESULT — the scorer is LOAD-BEARING, and "r~0 means noise" was WRONG.** Randomising role 3
(slot order) while leaving both gates untouched, 5 seeds:
```
baseline            net 530,568  Cal 0.45  mdd -14.8  9/10 yrs
A3 random (median)  net 256,642  Cal 0.15  mdd -23.9  4.4/10   = 48.4% of baseline
seed spread 15,566 .. 440,687 (baseline beats ALL five draws)
```
Shuffling the pick order halves net and cuts Calmar by two-thirds, consistently across every
seed. **So `wl_score` RANKING carries signal even though its LEVEL does not predict returns** —
`r=+0.037` was a misleading summary statistic, and the ㉗ mechanism story needs this correction.
**Do NOT delete the scorer.** Statistical honesty: n=5, and P(baseline > all 5 draws | null) ≈ 0.17,
so this is a large effect size rather than a p<0.05 proof. More seeds would settle it.

**The threshold roles pointed the other way** (full window): dropping role 2 gave net 592,361 /
Cal 0.49; dropping role 1 gave net **918,673 / Cal 0.60 / 9-of-10 yrs**, and appeared to fix the
era decay (**2022-26 net 39,474 → 239,877, 6x**). A4 (all three off) was poor precisely because it
discards the good role along with the bad ones (net 126,941 / Cal 0.09).

**★ P11 — the role-1 candidate FAILED the bar. Same shape as `b200->80`.**
```
floor    n      net       mdd   Calmar  yrs+    2015-21    2022-26
   45   265   530,568   -14.8    0.45   9/10    491,093     39,474   (shipped)
   35   273   412,997   -15.9    0.35   8/10    356,476     56,521   <- WORSE than shipped
   25   292   955,052   -16.1    0.62   9/10    698,928    256,124   <- peak
   15   291   918,673   -16.1    0.60   9/10    678,796    239,877
    5   291   918,673   -16.1    0.60   9/10    678,796    239,877   <- identical
    0   291   918,673   -16.1    0.60   9/10    678,796    239,877   <- identical
```
* **PLATEAU ✗** — non-monotone. Peak 0.62 at floor 25, but neighbour 35 holds only 56% of it and
  is *worse than shipped*. Same chaotic-surface signature that rejected the hysteresis arm.
* **SATURATION** — floors 15/5/0 are byte-identical: the knob stops binding below ~20, so P10's
  "floor=0" was really "floor≈20", and the bottom "plateau" is **one point measured three times**.
* **COST ✓** — floor 25 at 3x slippage: Cal 0.48, retains 78% of its 1x net, still above the
  shipped 0.45 at 1x. Genuinely not a thin-edge artefact.
* **OOS ✗ — the killer.** `floor=25` IS 0.70 → OOS **0.53**; `floor=45` IS **0.94** → OOS **0.54**.
  **Out of sample it is a dead heat with shipped, marginally behind**, and in-sample shipped is far
  better with half the drawdown (mdd -9.1 vs -16.0). The entire full-window advantage does not
  exist out of sample.
* **Mechanism note:** n rises only 265→292 (+10%) while net rises +80% ⇒ these are *different*
  trades, not more of the same. Changing the candidate pool changes which names top the `wl_score`
  ranking, which shifts slot allocation — consistent with the P10 finding that ordering is what
  matters.

**Verdict: 2 of 4 checks fail, including the two that matter most. Rejected.** A candidate that
fails plateau + OOS is not promoted on the checks it did pass — `b200->80` also survived cost
stress before dying the same way.

**⇒ 59 arms, 14 hypotheses, ZERO surviving improvements.** ㉗'s conclusion stands unchanged:
**swing is correctly built and correctly tuned; the constraint is REGIME, not configuration.**
The remaining swing decision is portfolio allocation, not engineering.

**Also established this session (independent of the arms):**
1. **A real harness bug** — the `emit_floor` PARAMETER never reaches role 2. Line 575 binds it to a
   local (`_emit_floor`, role 1) while line 761 reads the MODULE GLOBAL `EMIT_FLOOR`. So
   `emit_floor=N` silently leaves the adj_score gate at 45, and **the harness's own T4 "EMIT_FLOOR
   sweep" was sweeping the component pre-filter, not the score threshold.** Any earlier conclusion
   drawn from T4 about the score gate measured the wrong knob. Role 2 requires `H.EMIT_FLOOR`.
2. **Baseline reproduces exactly, 5x across independent processes** (`n=265 / net=530,568`, zero
   drift) — the harness is trustworthy, and both new knobs are provably inert.
3. **A performance diagnosis of mine that was WRONG.** I attributed the 462s/arm runtime to four
   per-symbol-day rolling recomputes (6.3M `statistics.median` calls etc.), precomputed all four
   into `Sym` — verified bit-identical on 42,172 indices for 3 of them, ~4e-10 float noise on
   `sma200` — and the runtime did not move (460s vs 462s). **The rolling stats were not the
   bottleneck.** The cost is almost certainly `compute_indicators`/`score_signal` on pre-filter
   survivors, which are prod's own domain functions and therefore off-limits to optimise. The
   precomputes were kept (harmless, gate-verified) but they buy nothing. Lesson: profile before
   optimising; op-count reasoning produced a confident wrong answer.
4. **Two prior backtest systems are unused by this line of work** — `src/autotrader/backtest/`
   (16 modules) and `backtest_v2/` (40 modules, incl. `swing_portfolio_sim`/`swing_alpha_finder`).
   `swing_final.py` imports only `domain/*`, `backtest.costs` and `settings` — deliberately, since
   fidelity requires calling prod's own functions. Cost: the 2.6 GB cache stores raw bars but **no
   derived features**, and there is no scan/simulate boundary, so every arm re-derives 11 years of
   candidates (2,825 days x 2,244 symbols = 6.3M evaluations) even when the arm only changes slot
   ordering. That is the real reason grinds are slow.
5. **Local compute hazard, self-inflicted** — earlier BQ-free analysis in the same session left the
   machine at 9.7/11.3 GB swap with 5.2M pageouts, which made one identical arm take 3033s instead
   of 462s (6.6x). Same failure mode as the 2026-06-15 laptop hang, even though this was
   single-process. Restart before long grinds.

**★ P12 / P12b — role 2 (the adj_score gate) given the SAME bar. Also rejected, but read the
caveat below before trusting the stamp.** Scripts `swing_p12_adjscore_plateau.py` (Phase A) and
`swing_p12b_adjscore_cost_oos.py` (Phases B+C, after P12 was killed mid-run by swap pressure).

PHASE A — and this one genuinely PASSED the plateau test, unlike role 1:
```
...
```
Flat and monotone (0.45 -> 0.49 -> 0.49), **identical maxDD**, same 9/10 years. No dip below
shipped anywhere. Knob saturates below ~30, so the usable range is only 45 -> 35.

PHASE B (3x slippage) and PHASE C (IS/OOS):
```
gate=45  slip 3x   Cal 0.45 -> 0.31   retains 73%
gate=35  slip 3x   Cal 0.49 -> 0.34   retains 69%
gate=45  IS 0.94 -> OOS 0.54
gate=35  IS 0.90 -> OOS 0.56
```

**⚠️ TWO OF MY OWN CHECKS WERE MIS-SPECIFIED. A future session must not read the "REJECTED"
stamp as if the bar were sound.**
1. **COST** required "Calmar at 3x above the shipped 0.45 at **1x**" — that compares a stressed
   run against an unstressed one. Shipped fails its own test (0.31 < 0.45). Like-for-like at 3x,
   **gate 35 (0.34) BEATS shipped (0.31)**.
2. **OOS** required "OOS not worse than IS by >15%" — but shipped degrades **identically**
   (0.94 -> 0.54 = −43% vs gate35's 0.90 -> 0.56 = −38%). That leg penalises the **era decay
   already documented in ㉗**, which is common to both configs, not a property of the candidate.

**On a fair like-for-like reading, gate=35 is marginally BETTER on every axis and worse on none:**
full-window Calmar 0.49 vs 0.45 · maxDD identical −14.8 · 3x-slip 0.34 vs 0.31 · **OOS 0.56 vs
0.54** · OOS years 4/5 tie.

**So why still reject? MULTIPLE COMPARISONS — and that is the whole argument.** This is arm ~65
across 15 hypotheses. Finding one arm with **+0.02 OOS Calmar** after 65 looks is precisely what a
garden-of-forking-paths search produces. Supporting evidence that it is noise: the entire effect
rests on a **2-trade difference** (n 265 -> 267); **maxDD is identical to the decimal**, so there
is no risk improvement; and +0.02 Calmar on **n=150 OOS trades** sits well inside sampling error.
A future session tempted by `gate=35` should demand a walk-forward, not a deploy — and should
weigh that 65 prior arms found nothing.

**⇒ FINAL: 65 arms, 15 hypotheses, ZERO surviving improvements.** ㉗'s conclusion stands:
**swing is correctly built and correctly tuned; the constraint is REGIME, not configuration.**
The remaining swing decision is portfolio allocation, not engineering.

**Runtime note (another wrong estimate of mine).** I predicted 35-40 min for P12b's 6 runs; it
took ~2 hours, because `gate=45 OOS` alone took **5404s** while its identical-shape twin took
**275s** — pure swap thrashing. The variance, not the arithmetic, is the whole uncertainty in any
runtime estimate on this machine.

**Also note:** even a passing role-1 result would NOT have been an env-var change — the component
pre-filter **is** the watchlist builder, so shipping it means modifying `universe_service`'s
watchlist construction, a shared prod module.

### ㉜ 2026-08-25 · Git commit identity was attributing this repo to the WRONG GitHub account
No prod change — repo config only. `autotrader` stays `00318-ws6`.

**Found:** every commit in this repo was authored `vishal@eloelo.in` (from the machine-global
`~/.gitconfig`), and **GitHub has that address linked to `vishal01012`** — not to `vishalrwt1995`,
which owns the repo. Proven, not guessed:
```
gh api repos/vishalrwt1995/python_auto_trader/commits/85b7dd8
  -> commit_email "vishal@eloelo.in"  linked_github_login "vishal01012"
```
Same wrong account behind the 2026-06-26 push-403 incident (see `feedback_github_identity.md`);
this is a second, quieter symptom of it — misattribution rather than an outright failure.

**Fixed (repo-local only):** `user.email = 229184469+vishalrwt1995@users.noreply.github.com` in
`<project>/.git/config` — the canonical no-reply form, numeric id fetched from the GitHub API
rather than guessed, so it attributes to `vishalrwt1995` without exposing a real address. Because
worktrees SHARE `.git/config`, this covers the main checkout and every `.claude/worktrees/*` at
once. **`~/.gitconfig` deliberately left at `vishal@eloelo.in`** — that is correct for the
voxy/eloelo/StoryTV work on this machine, and a global change would have broken it. Verified via
`git var GIT_AUTHOR_IDENT`, not assumed.

⚠️ **Do not "fix" this back.** A future session seeing a repo-local email that differs from global
should leave it alone — the difference is deliberate.

**History NOT rewritten.** All prior commits (incl. the 8 from 2026-08-25) remain attributed to
`vishal01012`. Rewriting pushed history + force-pushing `main` would break every worktree and
clone for pure attribution cosmetics. See §7 for the cleaner alternative.

**A framing I had to retract mid-task:** I first called this "the wrong identity" by conflating it
with the gcloud `--account=vishal@eloelo.in` rules stripped from the permission allowlist the same
day. Those are **Google Cloud auth for the voxy project** — unrelated to a git author email. The
concern turned out real, but only the GitHub API check established why.
