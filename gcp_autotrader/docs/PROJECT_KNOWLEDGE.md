...
**STATUS 2026-08-25: approved by the user; the `gcloud scheduler jobs update` call was blocked by the local
sandbox classifier (infrastructure write), so it must be applied by hand.** Not yet applied as of this entry —
verify `schedule: 30 19 * * *` before treating the gap as closed.
