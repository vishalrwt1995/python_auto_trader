...
| `nse_fii_deriv` / `nse_participant_oi` | FII/DII/Pro/Client index-fut long-short daily, 2015-26 (→06-19, ~1mo stale) | ~~FII-flow regime overlay / smart-money positioning~~ | **KILLED 2026-07-21** (diag ~₹0: every variant — net sign, 20d-trend, Nifty-UP×FII 2×2 — works IS then REVERSES OOS; regime-unstable [FII→DII marginal-buyer shift post-2021]; would WORSEN the price gate OOS. No cash flow / no per-stock cross-section in the data. Only both-halves+ cell [Nifty-DOWN×FII-long contrarian] is OOS n=40, un-buildable) |
...
4. **Fundamental Growth/Quality** — survivorship + arbitrage headwinds (★ long-shot); largest untapped cached set. Next cached candidate if pursued.
...
**State (2026-07-21):** both ★★ cached candidates (buyback, FII/DII) KILLED cheaply. The high-odds informed-accumulation pattern in cached data is exhausted (captured by the 3 live channels: delivery, insider, pledge). Remaining cached candidates (short-interest, fundamentals) are ★ long-shots. **The best remaining prize is Phase 2 SAST-accumulation (needs a new feed).**
