...
for i in $(seq 1 "${CLOSE_MAX_PASSES}"); do
  ...
  RESP="$(post_job "/jobs/score-cache-update-close?api_cap=600&lookback_days=700&min_bars=320&retry_stale_terminal_today=${RETRY_STALE}&run_intraday_update=true&intraday_api_cap=600&intraday_lookback_trading_days=60" || true)"
  echo "${RESP}"
