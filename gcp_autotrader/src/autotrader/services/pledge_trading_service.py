...
def _resolve_instrument_keys(symbols: Sequence[str], bq) -> dict[str, str]:
    ...
    try:
        ...
        logger.error("pledge_resolve_keys_deep_failed err=%s", exc)
    try:
        q = (f"SELECT symbol, ANY_VALUE(instrument_key) ik "
        ...
        logger.error("pledge_resolve_keys_fresh_failed err=%s", exc)
    merged = {**deep, **fresh}                      # fresh wins on conflict
    logger.info("pledge_resolve_keys asked=%d deep=%d fresh=%d merged=%d fresh_only=%d",
