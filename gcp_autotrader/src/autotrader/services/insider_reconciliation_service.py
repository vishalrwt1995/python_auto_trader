"""INSIDER channel reconciliation — daily premarket MAX-HOLD exit + SL-breach backstop.

Channel-isolated mirror of delivery/pead reconciliation for ``wl_type="insider"``, but
DELIBERATELY SIMPLER: the validated insider edge is a FIXED 90-session hold (a trailing stop
whipsaws this slow drift — proven in the grind), so there is NO 1R trail ratchet. This job only:
  1. exits if trading-days-held >= ``insider_max_hold_days`` (90)
  2. conservative daily-close SL-breach backstop (fixed 2.5xATR protective stop set at entry;
     can only exit later/worse than the backtest, never earlier — does not inflate results)

The intraday resting stop (paper-GTT / ws_monitor) is written at entry by order_service and is
NOT ratcheted here (fixed hold). Selects ONLY ``wl_type="insider"`` positions, which swing/
delivery/pead reconciliation exclude — so no other channel's exit path is touched. The decision
core (``evaluate_insider_position``) is pure + unit-tested; the wrapper does Firestore/Upstox/
order I/O (validated in PAPER). Called from ``/jobs/insider-reconcile`` premarket (~09:00 IST).

Validated config (2026-07-20): hold 90 sessions, ATR14x2.5 protective stop, NO trail.
"""
...
def evaluate_insider_position(
    *,
    ...
    """PURE: decide the exit action for one INSIDER position from fresh daily bars.

    Returns ``{exit_reason, days_held}``. ``exit_reason`` is None while the position stays
    open (fixed hold, no trail — the SL is unchanged from entry). Fixed 90-session hold +
    daily-close SL-breach backstop only.
    """
...
class InsiderReconciliationService:
    """Daily premarket max-hold/SL-breach exit manager for the INSIDER channel (PAPER-safe)."""
    ...
    def run(self, asof: str | None = None) -> InsiderReconcileResult:
        from autotrader.time_utils import now_ist
        result = InsiderReconcileResult()
        asof = asof or now_ist().strftime("%Y-%m-%d")
        cfg = self.settings.strategy
        open_positions = [p for p in self.state.list_open_positions()
                          if str(p.get("wl_type") or "").strip().lower() == "insider"]
        if not open_positions:
            logger.info("insider_recon no open insider positions")
            return result
        ...
        for pos in open_positions:
            ...
            try:
                ...
                if len(candles) < 2:
                    ...
                    continue
                ev = evaluate_insider_position(
                    entry_price=float(pos.get("entry_price") or 0),
                    side=str(pos.get("side") or "BUY"),
                    sl_price=float(pos.get("sl_price") or 0),
                    entry_ts=str(pos.get("entry_ts") or ""),
                    candles=candles,
                    max_hold_days=cfg.insider_max_hold_days,
                )
                if ev["exit_reason"]:
                    self._close(pos, tag, instrument_key, ev["exit_reason"], candles[-1][4], result)
                    time.sleep(0.05)
            except Exception:
                logger.exception("insider_recon_error tag=%s symbol=%s", tag, symbol)
                result.errors += 1
        logger.info("insider_recon_summary %s", result.to_dict())
        return result
    ...
    def _close(self, pos, tag, instrument_key, exit_reason, last_close, result):
        try:
            ...
            logger.exception("insider_recon_exit_order_failed tag=%s", tag)
            amo = False
        if not amo:
        ...
        logger.info("insider_recon_exit tag=%s reason=%s amo=%s", tag, exit_reason, amo)
