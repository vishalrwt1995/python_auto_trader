"""DELIVERY channel reconciliation — daily premarket trail + max-hold exit.

Channel-isolated mirror of swing_reconciliation_service / pead_reconciliation_service for
``wl_type="delivery"``:
  1. exit if trading-days-held >= ``delivery_max_hold_days`` (20)
  2. ratchet the daily 1R trailing stop (armed at +1.75R) via the SHARED
     ``swing_exit.trailed_stop`` — the single source of truth for the trail math, so
     delivery's trail can't drift from the validated geometry
...
It selects ONLY ``wl_type="delivery"`` positions, which swing_reconciliation excludes (it
matches ``wl_type=="swing"`` exactly) — so the swing/intraday/other-channel exit paths are
untouched. The decision core (``evaluate_delivery_position``) is pure + unit-tested; the
service wrapper does the Firestore/Upstox/order I/O (validated in PAPER). Called from
``/jobs/delivery-reconcile`` premarket (~09:00 IST), before the entry scan.

Validated config (2026-07-14): hold 20d, arm 1.75R, trail 1.0R, ATR14x2.5 stop.
"""
...
def evaluate_delivery_position(
    *,
    entry_price: float,
    side: str,
    sl_price: float,
    ...
    """PURE: decide the exit/trail action for one DELIVERY position from fresh daily bars.

    Returns ``{exit_reason, new_sl, peak, sl_moved, days_held}``. ``exit_reason`` is None
    when the position stays open (then ``new_sl`` is the ratcheted trailing stop). Mirrors
    swing/pead reconciliation logic exactly but parameterised for DELIVERY (20-day hold).
    Single source of truth for the trail: ``swing_exit.trailed_stop``.
    """
...
class DeliveryReconciliationService:
    """Daily premarket trail/exit manager for the DELIVERY channel (PAPER-safe)."""
    ...
    def run(self, asof: str | None = None) -> DeliveryReconcileResult:
        from autotrader.time_utils import now_ist
        result = DeliveryReconcileResult()
        asof = asof or now_ist().strftime("%Y-%m-%d")
        cfg = self.settings.strategy
        open_positions = [p for p in self.state.list_open_positions()
                          if str(p.get("wl_type") or "").strip().lower() == "delivery"]
        if not open_positions:
            logger.info("delivery_recon no open delivery positions")
            return result
        ...
        for pos in open_positions:
            ...
            try:
                ...
                ev = evaluate_delivery_position(
                    ...
                    candles=candles,
                    max_hold_days=cfg.delivery_max_hold_days,
                    trail_r=cfg.delivery_trail_r,
                    activate_r=cfg.delivery_activate_r,
                ...
                if abs(ev["new_sl"] - cur_sl) > 0.01 or ev["sl_moved"] != bool(pos.get("sl_moved")):
                    ...
                    try:
                        ...
                        logger.warning("delivery_recon_gtt_refresh_failed tag=%s", tag, exc_info=True)
                    result.updated_sl += 1
                    result.details.append({"tag": tag, "symbol": symbol, "action": "trail",
                                           "sl": ev["new_sl"], "days_held": ev["days_held"]})
            except Exception:
                logger.exception("delivery_recon_error tag=%s symbol=%s", tag, symbol)
                result.errors += 1
        logger.info("delivery_recon_summary %s", result.to_dict())
        return result
    ...
    def _close(self, pos, tag, instrument_key, exit_reason, last_close, result):
        try:
            ...
            logger.exception("delivery_recon_exit_order_failed tag=%s", tag)
            amo = False
        if not amo:
        ...
        logger.info("delivery_recon_exit tag=%s reason=%s amo=%s", tag, exit_reason, amo)
