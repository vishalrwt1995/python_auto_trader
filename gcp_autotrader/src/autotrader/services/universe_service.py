...
class UniverseService:
    ...
    def _universe_doc_to_row(self, doc: dict[str, Any]) -> list[Any]:
        ...
        row[22] = fs(doc.get("bars_1d"), "")
        row[23] = fs(doc.get("last_1d_date"), "")
        row[24] = fs(doc.get("price_last"), "")
        row[25] = fs(doc.get("turnover_med_60d"), "")
        row[26] = fs(doc.get("atr_14"), "")
        row[27] = fs(doc.get("atr_pct_14d"), "")
        row[28] = fs(doc.get("gap_risk_60d"), "")
        row[29] = fs(doc.get("turnover_rank_60d"), "")
        row[30] = fs(doc.get("liquidity_bucket"), "")
        row[31] = fs(doc.get("data_quality_flag"), "")
        row[32] = fs(doc.get("stale_days"), "")
